...
declare variable $extraction:version := "20180419-OR";
...
declare
%rest:GET
%rest:path("/extractions-full/{$enquete}")
function extraction:extraction-enquete-full($enquete as xs:string*) as node()* 
{
let $log := util:log("INFO", "extraction:extraction-enquete-full[$enquete="||$enquete||"]")  
let $racine-enquete := '/db/orbeon/fr/'||$enquete
let $enqueteToken := tokenize(upper-case($enquete),'-')
let $tmp-enquete := xmldb:create-collection("/db/tmp",$enquete)
let $collection-model := concat($racine-enquete,'/v1/data/')
let $temp:=
for $hash-reponse in xmldb:get-child-collections(concat($collection-model,'init/'))         
    let $collection-reponse := concat($collection-model,$hash-reponse)
    return
    if(xmldb:collection-available($collection-reponse))
        then(
            for $reponse in xmldb:get-child-resources($collection-reponse)
                let $doc:=doc($reponse)//form
                let $form-init:=<form>{$doc/@*}</form>
                return xmldb:store($tmp-enquete, $reponse||".xml",$form-init)
        )
        else()
let $zip-file := compression:zip(xs:anyURI($tmp-enquete),false())
let $zip-name:=$enquete||"-"||format-dateTime(current-dateTime(),"[Y,4][M,2][D,2][H,2][m,2][s,2]"||".zip")
let $sto := xmldb:store("/db/tmp",$zip-name,$zip-file,'application/zip')
 (:let $remove := xmldb:remove($tmp-enquete):)
return (common:rest-response(200,"zip des réponses au format XML"),$form-init)
};
...
declare
%rest:GET
%rest:path("/extractions/reporting/{$enquete}")
%rest:query-param("date", "{$date}","")
function extraction:getSuiviTest($enquete as xs:string*,$date as xs:string*) as item()*
{
let $log := util:log("INFO", "extraction:getExpedie[$enquete="||$enquete||"]") 
let $racine-enquete := '/db/orbeon/fr/'||$enquete||'/'
return try{
...
for $model in xmldb:get-child-collections($racine-enquete)
    ...
    let $dateparam := if ($date eq "") then (datetime:parse-dateTime('20100101000000',"yyyyMMddhhmmss")) else (datetime:parse-dateTime($date||'00',"yyyyMMddhhmmss"))
        ...
        for $hash-reponse in xmldb:get-child-collections($collection-model)         
            ...
            for $init in xmldb:get-child-resources($collection-init)
                ...
                let $reponseAvalaible := doc-available($cheminReponse)
                let $docReponse := if($reponseAvalaible) then(doc($collection-model-reponse||$hash-reponse||'/'||$init)) else('')            
                let $rez := if($reponseAvalaible) then (
                    let $dateReponse := $docReponse//util/dateHeure/text()
                    (: <dateHeure>01-03-2019 à 10:21</dateHeure> :)
                    let $annee := substring($dateReponse,7,4)
                    let $mois := substring($dateReponse,4,2)
                    let $jour := substring($dateReponse,1,2)
                    let $heure := substring($dateReponse,14,2)
                    let $minute := substring($dateReponse,17,2)
                    let $dateReponseFormat := datetime:parse-dateTime($annee||$mois||$jour||$heure||$minute||'00',"yyyyMMddhhmmss")    
                       
                    let $envoi :=($dateReponseFormat - $dateparam > xs:dayTimeDuration('PT0S'))
                    
                    return if ($envoi) then (
                    <Suivi idUniteEnquetee='{substring-before($init,'.xml')}' idModele='{$model}' hashRepertoire='{$hash-reponse}'>
                        <reponse>oui</reponse>
                        <expedie>{$docReponse//util/expedie/text()}</expedie>
                        <dateHeure>{$docReponse//util/dateHeure/text()}</dateHeure>
                        <extrait>{$docReponse//util/extrait/text()}</extrait>                                                                   
                        </Suivi>)
                        else ()
                    ) 
                    else ()
